// /app/api/settings/specialties/[id]/route.ts
...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const dup = await SpecialtiesMaster.findOne({ _id: { $ne: id }, name: { $regex: `^${name}$`, $options: "i" } });
    if (dup) {
      return error("Duplicate entry.", { name: "Another record with same name exists." }, 409);
    }

    const updated = await SpecialtiesMaster.findByIdAndUpdate(id, { name }, { new: true });
    if (!updated) return error("Not found.", { id: "Not found." }, 404);

    return success("Updated successfully.", updated);
  } catch (err: any) {
    console.error("PUT /api/settings/specialties/[id] error:", err);
    if (err?.code === 11000) {
      return error("Duplicate entry.", { name: "This specialtie already exists." }, 409);
    }
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const removed = await SpecialtiesMaster.findByIdAndDelete(id);
    if (!removed) return error("Not found.", { id: "Not found." }, 404);

    return success("Deleted successfully.", { id });
  } catch (err: any) {
    console.error("DELETE /api/settings/specialties/[id] error:", err);
    return error("Server error.", { server: err.message }, 500);
  }
}
