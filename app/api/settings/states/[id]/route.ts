...
export async function PUT(
  ...
  try {
    ...
    if (dup) {
      return error(
        "Duplicate entry.",
        { code: "Another state with this code exists for this country." },
        409
      );
    }
    ...
    return success("State updated.", updated);
  } catch (err: any) {
    console.error("PUT /api/settings/states/[id] error:", err);
...
export async function DELETE(
  ...
  try {
    ...
    const deleted = await StateMaster.findByIdAndDelete(id);
    if (!deleted) {
      return error("Not found.", { id: "Record not found" }, 404);
    }

    return success("State deleted.", { id });
  } catch (err: any) {
    console.error("DELETE /api/settings/states/[id] error:", err);
    return error("Server error.", { server: err.message }, 500);
  }
}