// /app/api/settings/routes-of-administration/[id]/route.ts
...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const dup = await RouteOfAdministrationMaster.findOne({
      _id: { $ne: id },
      code: codeRaw,
    });
    if (dup) {
    ...
    const updated = await RouteOfAdministrationMaster.findByIdAndUpdate(
      id,
      { code: codeRaw, name: nameRaw, description: descRaw },
      { new: true }
    );
    ...
    console.error("PUT /api/settings/routes-of-administration/[id] error:", err);
    if (err?.code === 11000) {
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const removed = await RouteOfAdministrationMaster.findByIdAndDelete(id);
    if (!removed) return error("Not found.", { id: "Not found." }, 404);

    return success("Deleted successfully.", { id });
  } catch (err: any) {
    console.error("DELETE /api/settings/routes-of-administration/[id] error:", err);
    return error("Server error.", { server: err.message }, 500);
  }
}