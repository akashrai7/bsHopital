// /app/api/settings/vaccine-sites/route.ts
import { connectMongo } from "@/lib/mongoose";
import VaccineSiteMaster from "@/models/VaccineSiteMaster";
...
export async function GET(req: Request) {
  try {
    ...
    const docs = await VaccineSiteMaster.find(filter)
      .sort({ createdAt: -1 })
      .lean();

    return success("Fetched vaccine sites.", docs);
  } catch (err: any) {
    console.error("GET /api/settings/vaccine-sites error:", err);
...
export async function POST(req: Request) {
  try {
    ...
    const existing = await VaccineSiteMaster.findOne({ code: codeRaw });
    if (existing) {
      return error("Duplicate entry.", { code: "This code already exists." }, 409);
    }

    const doc = await VaccineSiteMaster.create({
      code: codeRaw,
      name: nameRaw,
      description: descRaw,
    });

    return success("Vaccine site created.", doc);
  } catch (err: any) {
    console.error("POST /api/settings/vaccine-sites error:", err);
    if (err?.code === 11000) {
      return error("Duplicate entry.", { code: "This code already exists." }, 409);
    }
    return error("Server error.", { server: err.message }, 500);
