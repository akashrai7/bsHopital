// /app/api/settings/blood-groups/[id]/route.ts
...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    // check duplicate on code except same id
    const dup = await BloodGroupMaster.findOne({
      _id: { $ne: id },
      code: codeRaw,
    });
    if (dup) {
      return error("Duplicate entry.", { code: "Another record with same code exists." }, 409);
    }

    const updated = await BloodGroupMaster.findByIdAndUpdate(
      id,
    ...
    console.error("PUT /api/settings/blood-groups/[id] error:", err);
    if (err?.code === 11000) {
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const removed = await BloodGroupMaster.findByIdAndDelete(id);
    if (!removed) return error("Not found.", { id: "Not found." }, 404);

    return success("Deleted successfully.", { id });
  } catch (err: any) {
    console.error("DELETE /api/settings/blood-groups/[id] error:", err);
    return error("Server error.", { server: err.message }, 500);
  }
}
