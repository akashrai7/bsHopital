// /app/api/settings/countries/[id]/route.ts
...
export async function PUT(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const dup = await CountryMaster.findOne({
      _id: { $ne: id },
      code: codeRaw,
    });
    if (dup) {
    ...
    console.error("PUT /api/settings/countries/[id] error:", err);
    if (err?.code === 11000) {
...
export async function DELETE(req: Request, { params }: { params: { id: string } }) {
  try {
    ...
    const removed = await CountryMaster.findByIdAndDelete(id);
    if (!removed) return error("Not found.", { id: "Not found." }, 404);

    return success("Deleted successfully.", { id });
  } catch (err: any) {
    console.error("DELETE /api/settings/countries/[id] error:", err);
    return error("Server error.", { server: err.message }, 500);
  }
}
