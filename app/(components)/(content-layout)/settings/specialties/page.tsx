...
export default function SpecialtiesPage() {
  const [list, setList] = useState<SpecialtieItem[]>([]);
  ...
  async function fetchList() {
    setLoading(true);
    try {
      const res = await fetch("/api/settings/specialties", { method: "GET" });
     ...
     // console.error("Fetch specialties error:", err);
      toast.error("Server error while fetching specialties");
    } finally {
      setLoading(false);
    }
  }
  ...
  async function handleSubmit(e?: React.FormEvent) {
    ...
    try {
      ...
      const url = editingId
        ? `/api/settings/specialties/${editingId}`
        : "/api/settings/specialties";
      ...
      if (!data?.status) {
        ...
        toast.success(editingId ? "specialtie updated" : "specialtie added");
        setForm({ name: "" });
        setEditingId(null);
        fetchList();
      }
  ...
  function handleEdit(item: SpecialtieItem) {
    setForm({ name: item.name });
    setEditingId(item._id);
    window.scrollTo({ top: 0, behavior: "smooth" });
  }

  async function handleDelete(item: SpecialtieItem) {
    const ok = confirm(`Are you sure you want to delete "${item.name}"?`);
    if (!ok) return;
    try {
      const res = await fetch(`/api/settings/specialties/${item._id}`, { method: "DELETE" });
      const data = await res.json();
      if (!data?.status) {
      ...
      console.error("Delete specialtie error:", err);
  ...
  return (
    <Fragment>
      <Seo title="Manage Specialties" />
      <Pageheader title="Settings" currentpage="Specialties" activepage="Specialty" />

      <Row>
        <Col xl={6}>
          <Card className="custom-card">
            <Card.Header>
              <Card.Title>{editingId ? "Edit specialtie" : "Add specialtie"}</Card.Title>
            </Card.Header>
            <Card.Body>
                ...
                <div className="mb-3">
                  ...
                  <InputGroup>
                    <Form.Control
                      ...
                      type="text"
                      placeholder="Enter specialtie name (e.g., Cardiology)"
        ...
        <Col xl={6}>
          <Card className="custom-card">
            <Card.Header>
              <Card.Title>Specialtie List</Card.Title>
            </Card.Header>
            <Card.Body>
