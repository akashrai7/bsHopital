...
export default function CountryPage() {
  ...
  async function handleSubmit(e?: React.FormEvent) {
    ...
    try {
      ...
      const url = editingId
        ? `/api/settings/countries/${editingId}`
  ...
  function handleEdit(item: CountryItem) {
  ...
  async function handleDelete(item: CountryItem) {
    ...
    if (!ok) return;
    try {
      const res = await fetch(`/api/settings/countries/${item._id}`, {
        method: "DELETE",
      });
      const data = await res.json();
      if (!data?.status) {
      ...
      <Row>
        ...
        <Col xl={4}>
          <Card className="custom-card">
            <Card.Header>
              <Card.Title>{editingId ? "Edit Country" : "Add Country"}</Card.Title>
            </Card.Header>
            <Card.Body>
